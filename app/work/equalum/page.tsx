...
export async function generateMetadata(): Promise<Metadata> {
  const project = getProject('equalum');
  if (!project) {
...
export default function EqualumWorkPage() {
  const project = getProject('equalum');
  if (!project) {
    notFound();
  }
  return (
    <>
      ...
      {/* Why It Matters section, only if present */}
      {project.whyItMatters && (
        <ContentSection
          heading="Why It Matters"
          content={project.whyItMatters.replace(/^Why It Matters\n?/, '')}
          layout="text-only"
          theme={project.theme}
        />
      )}
